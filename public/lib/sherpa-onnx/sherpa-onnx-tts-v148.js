if (typeof window !== 'undefined') {
  window.__SHERPA_TTS_BRIDGE_VERSION__ = "1.4.8";
  console.log("%c >>> SherpaONNX Bridge v1.4.8: DEFINITIVE ALIGNMENT <<< ", "background: #2e7d32; color: #fff; font-size: 16px; font-weight: bold;");
}

function copyHeap(Module, src, len, dst) {
  if (Module._CopyHeap) {
    console.log(`SherpaONNX: Copying ${len} bytes from ${src} to ${dst} via _CopyHeap`);
    Module._CopyHeap(src, len, dst);
  } else {
    console.log(`SherpaONNX: Copying ${len} bytes from ${src} to ${dst} via HEAPU8 fallback`);
...
function initSherpaOnnxOfflineTtsVitsModelConfig(config, Module) {
  ...
  console.log(`SherpaONNX v1.4.8: VITS aligned: ${config.model}, ${config.tokens}, ${config.dataDir}`);

  const n = modelLen + lexiconLen + tokensLen + dataDirLen;
  const buffer = Module._malloc(n);
  const len = 7 * 4; // model, lexicon, tokens, dataDir, noiseScale, noiseScaleW, lengthScale
...
function initSherpaOnnxOfflineTtsMatchaModelConfig(config, Module) {
  ...
  const n = acousticModelLen + vocoderLen + lexiconLen + tokensLen + dataDirLen;
  const buffer = Module._malloc(n);
  const len = 7 * 4; // model, vocoder, lexicon, tokens, dataDir, noiseScale, lengthScale
  const ptr = Module._malloc(len);
...
function initSherpaOnnxOfflineTtsKokoroModelConfig(config, Module) {
  ...
  const n = modelLen + voicesLen + tokensLen + lexiconLen + dataDirLen + langLen;
  const buffer = Module._malloc(n);
  const len = 7 * 4; // model, voices, tokens, lexicon, dataDir, lengthScale, lang
  const ptr = Module._malloc(len);
  ...
  Module.setValue(ptr + 12, buffer + offset, 'i8*'); offset += lexiconLen;
  Module.setValue(ptr + 16, buffer + offset, 'i8*'); offset += dataDirLen;
  Module.setValue(ptr + 20, config.lengthScale || 1.0, 'float');
  Module.setValue(ptr + 24, buffer + offset, 'i8*'); offset += langLen;
...
function initSherpaOnnxOfflineTtsModelConfig(config, Module) {
  ...
  copyHeap(Module, zipVoiceModelConfig.ptr, zipVoiceModelConfig.len, ptr + offset);
  ...
  copyHeap(Module, pocketModelConfig.ptr, pocketModelConfig.len, ptr + offset);
  offset += pocketModelConfig.len;

  copyHeap(Module, supertonicModelConfig.ptr, supertonicModelConfig.len, ptr + offset);
...
function initSherpaOnnxOfflineTtsConfig(config, Module) {
  ...
  let offset = 0;
  copyHeap(Module, modelConfig.ptr, modelConfig.len, ptr + offset);
  ...
  Module.setValue(ptr + offset, buffer, 'i8*'); // rule_fsts
  offset += 4;

  Module.setValue(ptr + offset, buffer + ruleFstsLen, 'i8*'); // rule_fars
  offset += 4;

  Module.setValue(ptr + offset, config.maxNumSentences || 1, 'i32'); // max_num_sentences
  offset += 4;

  Module.setValue(ptr + offset, config.silenceScale || 0.2, 'float'); // silence_scale
  offset += 4;
...
class OfflineTts {
  ...
  generate(config) {
    ...
    try {
      ...
      console.log(`SherpaONNX v1.4.8: TRACE: Audio Handle ${h} -> ptr=${samplesPtr}, n=${numSamples}, rate=${sampleRate}`);

      if (!samplesPtr || numSamples <= 0 || numSamples > 20000000) {
        throw new Error(`Invalid audio data (h=${h}, ptr=${samplesPtr}, n=${numSamples})`);
      }
      ...
      console.error('SherpaONNX v1.4.8: Error reading generated audio:', e);
