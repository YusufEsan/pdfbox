...
function initSherpaOnnxOfflineTtsVitsModelConfig(config, Module) {
  ...
  console.log(`SherpaONNX JS (v1.2.7): VITS Config:
    model: ${config.model}
...
function initSherpaOnnxOfflineTtsModelConfig(config, Module) {
  ...
  copyHeap(
      Module, zipVoiceModelConfig.ptr, zipVoiceModelConfig.len, ptr + offset);
  ...
  copyHeap(
      Module, supertonicModelConfig.ptr, supertonicModelConfig.len, ptr + offset);
...
class OfflineTts {
  ...
  generate(config) {
    ...
    console.log(`SherpaONNX JS: Generating with sid=${config.sid ?? 0}, speed=${config.speed ?? 1.0}, textLen=${config.text.length}`);
    ...
    // Use ccall for safer invocation
    const h = this.Module.ccall(
    ...
    console.log(`SherpaONNX JS: Generation returned pointer: ${h}`);

    if (!h) {
      console.error('SherpaONNX JS: Generation returned null handle');
      throw new Error('TTS generation failed (null handle)');
    }

    try {
      const samplesPtr = this.Module.getValue(h, 'i8*'); // float* samples
      const numSamples = this.Module.getValue(h + 4, 'i32'); // int32_t num_samples
      const sampleRate = this.Module.getValue(h + 8, 'i32'); // int32_t sample_rate

      console.log(`SherpaONNX JS: Decoded audio: samplesPtr=${samplesPtr}, numSamples=${numSamples}, sampleRate=${sampleRate}`);
      ...
      this.Module.ccall('SherpaOnnxDestroyOfflineTtsGeneratedAudio', null, ['number'], [h]);
      return {samples: samples, sampleRate: sampleRate};
    } catch (e) {
      console.error('SherpaONNX JS: Error reading generated audio:', e);
      if (h) this.Module.ccall('SherpaOnnxDestroyOfflineTtsGeneratedAudio', null, ['number'], [h]);
      throw e;
    }
  }
