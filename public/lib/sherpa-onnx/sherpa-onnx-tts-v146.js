if (typeof window !== 'undefined') {
  window.__SHERPA_TTS_BRIDGE_VERSION__ = "1.4.6";
  console.log("%c >>> SherpaONNX Bridge v1.4.6: THE FINAL STAND <<< ", "background: #c62828; color: #fff; font-size: 16px; font-weight: bold;");
}
...
class OfflineTts {
  ...
  generate(config) {
    ...
    try {
      ...
      console.log(`SherpaONNX v1.4.6: TRACE: Audio Handle ${h} -> ptr=${samplesPtr}, n=${numSamples}, rate=${sampleRate}`);

      if (!samplesPtr || numSamples <= 0 || numSamples > 20000000) {
        throw new Error(`Invalid audio data: h=${h}, n=${numSamples}`);
      }
      ...
      console.error('SherpaONNX v1.4.6: TRACE: Error reading generated audio:', e);
