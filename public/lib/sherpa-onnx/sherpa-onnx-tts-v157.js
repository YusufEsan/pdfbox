(function() {
  if (typeof window !== 'undefined') {
    window.__SHERPA_TTS_BRIDGE_VERSION__ = "1.5.7";
    console.log("%c >>> SherpaONNX Bridge v1.5.7: MEMORY & ALIAS ALIGNMENT <<< ", "background: #1e88e5; color: #fff; font-size: 16px; font-weight: bold;");
  }
  ...
  function initSherpaOnnxOfflineTtsModelConfig(config, Module) {
    ...
    // VITS(32) + num_threads(4) + debug(4) + provider(4) + matcha(32) + kokoro(32) + kitten(16) + zipvoice(40) + pocket(32) + supertonic(28)
    const totalLen = 32 + 3*4 + 32 + 32 + 16 + 40 + 32 + 28;
    const ptr = Module._malloc(totalLen);
    
    // Safety: Zero out the entire model config struct first
  ...
  function initSherpaOnnxOfflineTtsConfig(config, Module) {
    ...
    // Safety: Zero out the entire tts config struct
    Module.HEAPU8.fill(0, ptr, ptr + totalLen);
  ...
  class OfflineTts {
    ...
    generate(config) {
      ...
      console.log(`[TRACED] Generating: "${config.text.substring(0, 30)}..." sid=${config.sid} speed=${config.speed}`);
      const h = this.Module._SherpaOnnxOfflineTtsGenerate(this.handle, tPtr, config.sid || 0, config.speed || 1.0);
      this.Module._free(tPtr);
      if (!h) throw new Error('TTS failed (Handle returned null)');
      const base = h / 4;
